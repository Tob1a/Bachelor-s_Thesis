...
Lo studio proposto in questa tesi espone un metodo per identificare e contare il numero di piante di avocado in un terreno aziendale tra la Spagna e il Portogallo attraverso tecniche di machine learning basate su immagini satellitari.\\
Lo scopo è fornire un algoritmo che attraverso un'immagine che sia in grado di individuare il numero di piante, e anche di conseguenza la dimensione totale della piantagione, per una rapida verifica del controllo del territorio agricolo.
...
\setupfancy
\thispagestyle{empty}
In questo capitolo si affronterà le basi per comprendere successivamente appieno la struttura e i passaggi del lavoro svolto.
...
\setupfancy
\thispagestyle{empty}
In questo capitolo si andrà ad analizzare i dati e la loro struttura e successivamente si faranno delle scelte riguardo la creazione del modello.
infine si mostrerà la creazione del modello stesso usando il lavoro descritto nel capitolo precedente.

\input{capitoli/capitolo2/capitolo_2}
...
\begin{table}[H]
  ...
  \begin{tabular}{lS[table-format=1.2]S[table-format=1.2]S[table-format=1.2]S[table-format=6.0]}
    ...
    \bottomrule
  \end{tabular}
  \caption{Risultati per classe — Cross-Validation sul Training Set (3 classi)}
...
\begin{table}[H]
  ...
  \begin{tabular}{lS[table-format=1.2]S[table-format=1.2]S[table-format=1.2]S[table-format=6.0]}
    ...
    \bottomrule
  \end{tabular}
  \caption{Risultati per classe — Cross-Validation sul Training Set (4 classi)}
...
salvati gli agglomerati, che la Foresta Casuale identifica come pianta, e 
...
\begin{itemize}
  \item Non riescono a risolvere in maniera precisa la \texttt{tipologia3} e la \texttt{tipologia2}, poiché il modello tende ad associare le piante ad un'altra tipologia. 
...
\end{itemize}
Questo tipo di approcci non ha soluzioni empiriche universali ma ogni caso richiede un adattamento specifico. 
In futuro, si potrebbe creare un ampliamento del dataset su cui lavorare e un'integrazione di bande infrarosso aggiuntive e un accrescimento di risoluzione per immagini su cui poter andare a lavorare.
...
Soluzioni analoghe verranno sempre più utilizzate in futuro man mano che la tecnologia avanzerà e 
verranno sempre più usate tecnologie di precisione di larga scala 
per l'agricoltura.
