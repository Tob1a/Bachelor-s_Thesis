...
Durante la ricerca per trovare dei dataset gratuiti utilizzabili per l'addestramento del modello
oltre a quelli messi a disposizione dell'azienda si è trovato il dataset 
\cite{guillem_montalban_faet_2025_17866344} che tuttavia non contiene il canale SWIR e le cui immagini sono state  scattate da un drone e non da un satellite.
...
Per valutare la bontà dei dati e se un ipotetico classificatore è in grado di distinguere i vari pixel si sono costruiti i grafici dei valori delle bande.
...
Come si può osservare dai seguenti grafici \ref{fig:grafici SWIR pianta12} \ref{fig:grafici SWIR pianta12 senza0} \ref{fig:grafici NIR pianta12} \ref{fig:grafici NIR pianta12 senza0} tutte le tipologie 
di piante analizzate è possibile distinguere visivamente le distribuzioni di tutte le tipologie.

Si afferma inoltre lo sbilanciamento del dataset per le classi. Si riporta una tabella che mostra il numero di tipologie
\begin{table}[H]
\centering
...
\end{table}
Questo afferma che le proporzioni del dataset non sono distribuite
...
Per i compiti di classificazione nelle 3 tipologie di piante si è scelto di adottare una Foresta Casuale \ref{subsec:foresta_casuale}.  
Rispetto all'utilizzo di una CNN (Convolutional Neural Network) per la classificazione di ogni pixel, la Foresta Casuale 
offre maggiore interpretabilità dei dati, richiede un numero di dati minore rispetto una CNN per l'allenamento, 
...
Poichè il dataset \cite{guillem_montalban_faet_2025_17866344} lavora solo con la banda infrarossa NIR si è scelto di addestrare 2 Foreste Casuali. 
Una che lavorerà solo con la banda NIR e una che lavorerà sia con la banda NIR che con la banda SWIR.

\section{Implementazione del modello}
\subsection{Foresta Casuale su immagini}
Si è andati a creare delle features derivate: media3x3, media7x7, std3x3 e std7x7 per ogni banda disponibile (quindi NIR e SWIR). Nello specifico si crea un immagine .tiff con 10 bande (media3x3rosso,media3x3verde,...).
...
di 100 alberi per Foresta (infatti indipendentemente dal supermamento di questa soglia la bontà del modello non aumentava) e non è stato impostato un vincolo 
in profondità per gli alberi decisionali. Infatti non vi erano grossi differenze tra le profondità degli alberi della foresta e la loro performance.
...
Dato che una singola pianta è considerata come un agglomerato di pixel \texttt{pianta(di una singola tipologia)}, cioè un gruppo di pixel adiacenti, per risalire al numero di piante è necesssario lavorare sugli agglomerati stessi.\\
Un agglomerato è definito come un insieme massimale di pixel di classe \texttt{pianta} appartanenti ad una singola tipologia, in cui ogni pixel è connesso ad almeno un altro pixel dell'insieme tramite adiacenza. \\
Per estrarre tutti gli agglomerati dall'immagine, si esegue una scansione completa dell'immagine.
Per contare i pixel presente negli agglomerati si itera su tutti i pixel dell'immagine. Quando si trova un pixel pianta allora si esegue un BFS (Breadth First Search) dei pixel pianta vicini e si conta il numero dei pixel.
Si è pensato di effettuare attraverso il BFS anche la conta i pixel del perimetro dell'agglomerato.
Durante la ricerca per risolvere il problema si è pensato di usare una Regressione Lineare sugli agglomerati di pixel per comprendere il numero di piante presenti in quell'insieme.
Il plot della regressione per la \texttt{tipologia avocado} è il seguente:
\begin{figure}[htbp]
    \centering
...
accuratezza maggiore ($R^2=0.991$) rispetto il perimetro ($R^2=0.968$)\\
...
Questo modello è stato addestrando utilizzando lo stesso dataset del modello NIR e SWIR, però usando solo il canale NIR e il dataset \cite{guillem_montalban_faet_2025_17866344}.
La risoluzione satellitare del dataset fornito dall'azienda è di $30cm/px$. 
Non tutti dataset sono normalizzati quindi si procederà con una normalizzazione della risoluzione.
...
In questo modo si è potuto comprendere quanto ridimensionare l'immagine del dataset \cite{guillem_montalban_faet_2025_17866344} 
in modo da uniformarle al dataset satellitare fornito dall'azienda.

Si ricorda che quindi questo modello troverà una tipologia di pianta in più ovvero gli aranci presenti solo nel dataset \cite{guillem_montalban_faet_2025_17866344}.